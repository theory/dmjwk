...
[DMJWK_AUDIENCE](#dmjwk_audience). Optional.
...
If dmjwk starts with [DMJWK_ISSUER](#dmjwk_issuer) configured, validation will
require the token's `iss` field to contain this value. And if dmjwk starts
with [DMJWK_AUDIENCE](#dmjwk_audience) configured, validation will require the
token's `aud` field to contain at least one of its values. JWTs generated by
the [authorization](#post-authorization) API will contain the values of these
